...
class LocalModel(object):
    ...
    def train_model(self, early_stop=False, tenacity=10):
        ...
        while self.D.epoch < self.num_epochs:
            ...
            if self.test:
                if use_cuda:
                    inp_test = inp_test
                    out_target_test = out_target_test
    ...
    def convert_to_input(self, data, cuda=False):
        n, m = data.shape
    ...
    def convert_covariates(self, data, covs, cuda=False):
        nd, td = data.shape
        rcovs = np.repeat(
    ...
    def convert_ycovs(self, data, ycovs, cuda=False):
        nd, td = data.shape
